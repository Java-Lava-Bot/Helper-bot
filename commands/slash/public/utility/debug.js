...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        try {
            ...
            try {
                if (!interaction.replied && !interaction.deferred) {
                    await interaction.reply({ content: `An error occurred while running the command. Please report this to the support server ${supportinvite}`, flags: 64 });
                    ...
                    await interaction.followUp({ content: `An error occurred while running the command. Please report this to the support server ${supportinvite}`, flags: 64 });
                    logger.error(`[Stable Debug] Error executing command for ${interaction.user.tag}: ${err?.message ?? err}`, err);
                    LogError(err, interaction, 'commands/slash/public/utility/debug.js');
                }
            } catch (Err) {

  ...
  async execute(interaction) {
    ...
    try {
      if (sub === "setup") {
        ...
        setupBumpReminder(interaction.guild.id, channel.id, role?.id ?? null);
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({
            text: "Run /bump once in that channel to kick off the first reminder cycle.",
          })
          .setTimestamp();
      ...
      if (sub === "remove") {
        ...
        removeBumpReminder(interaction.guild.id);
      ...
      logger.error(`Error executing /bump-reminder ${sub}: ${error}`);
      LogError(error, interaction.client, "Slash Command: /bump-reminder");

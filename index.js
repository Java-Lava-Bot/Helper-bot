...
const registerTopGgWebhook = require("./top-gg");
...
async function InteractionHandler(interaction, type) {
  ...
  try {
    ...
    try {
      ...
      await interaction
        .editReply({
          content: `<:error:1444133000915976253> There was an error while executing this command!`,
          embeds: [],
          components: [],
          files: [],
        })
      ...
      if (code === 10062 || code === 40060) {
        // interaction already acknowledged / unknown — nothing to do
      } else {
    ...
    try {
      ...
      console.error("LogError failed while reporting execution error", e);
    }
  }
}
